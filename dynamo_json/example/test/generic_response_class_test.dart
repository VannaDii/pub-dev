...
void main() {
  test('user', () {
    _testResponse(BaseResponse<User>.fromDynamoJson(_jsonUser), _testUser);
    // without generic
    _testResponse(BaseResponse.fromDynamoJson(_jsonUser), _testUser);
  });

  test('article', () {
    _testResponse(
        BaseResponse<Article>.fromDynamoJson(_jsonArticle), _testArticle);
    // without generic
    _testResponse(BaseResponse.fromDynamoJson(_jsonArticle), _testArticle);
  });

  test('article list', () {
    _testResponse(
      BaseResponse<List<Article>>.fromDynamoJson(_jsonArticleList),
      _testArticleList,
    );

    // without generic
    _testResponse(
        BaseResponse.fromDynamoJson(_jsonArticleList), _testArticleList);
  });
}
