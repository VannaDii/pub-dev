...
final _dynamoKeyExpando = Expando<Map<ClassConfig, KeyConfig>>();

KeyConfig dynamoKeyForField(FieldElement field, ClassConfig classAnnotation) =>
    (_dynamoKeyExpando[field] ??= Map.identity())[classAnnotation] ??=
    ...
    if (enumFields != null) {
      if (mustBeEnum) {
        ...
        if (_nullAsUnknownChecker.isExactlyType(annotationType)) {
          return dynamoKeyNullForUndefinedEnumValueFieldName;
      ...
      return dynamoLiteralAsDart(defaultValueLiteral);
    }
  }
...
const dynamoKeyNullForUndefinedEnumValueFieldName =
